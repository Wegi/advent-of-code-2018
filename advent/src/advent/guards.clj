...
(defn parse-action
  [action]
  (let [guard (re-find #"Guard #(\d+)" action)
        wakes? (re-find #"wakes up" action)]
    (if guard
      (second guard)
      (if wakes?
        :wakes
...
(defn match-line
  [line]
  (let [match-regex #"\[(\d+)-(\d+)-(\d+) (\d+):(\d+)\] (.+)$"
        matcher (re-find match-regex line)
        line-map (zipmap [:year :month :day :hour :minute :action] (rest matcher))]
    (update line-map :action parse-action)))

(defn sort-inputs
  [input]
  (let [lines (str/split input #"\n")
        logs (map match-line lines)]
    (sort-by (juxt :year :month :day :hour :minute) logs)))

(defn calculate-sleep
  [input]
  (loop [logs (sort-inputs input)
         on-guard ""
         slept-since 0
         sleep-times {}]
    (if (seq logs)
      (let [current (first logs)
            minute (Integer/parseInt (:minute current))]
        (condp = (:action current)
          :sleeps (recur (rest logs) on-guard minute sleep-times)
          :wakes (recur (rest logs) on-guard slept-since
                        (update sleep-times on-guard conj (range slept-since minute)))
          (recur (rest logs) (:action current) slept-since sleep-times)))
      sleep-times)))

(defn max-sleeper
  [sleep-times]
  (let [total-sleep (into {} (map (fn [[guard times]] [guard (count (flatten times))]) sleep-times))]
    (Integer/parseInt (key (apply max-key val total-sleep)))))

(defn favorite-naptime
  [sleep-vecs]
  (->> sleep-vecs
       flatten
       frequencies
